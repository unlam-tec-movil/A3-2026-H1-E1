...
class GetClinicsStoredUseCaseTest {
    ...
    @Test
    fun `invoke returns flow of stored clinics`() = runTest {
        // Arrange
        val clinics = listOf(
            Clinic(
                id = 0,
    ...
    @Test
    fun `invoke returns empty flow when no clinics stored`() = runTest {
        // Arrange
    ...
    @Test
    fun `invoke returns flow that emits all clinics`() = runTest {
        // Arrange
        val clinics = (0..9).map { i ->
            Clinic(
                id = i,
                name = "Clinic $i",
                address = "Address $i",
    ...
    @Test
    fun `invoke returns single clinic in flow`() = runTest {
        // Arrange
        val clinic = Clinic(
            id = 5,
            name = "Single Clinic",
    ...
    @Test
    fun `invoke delegates to repository getStoredClinics`() = runTest {
        // Arrange
        every { mockRepository.getStoredClinics() } returns flowOf(emptyList())
    ...
    @Test
    fun `invoke preserves clinic data integrity`() = runTest {
        // Arrange
        val clinic = Clinic(
            id = 99,
    ...
    @Test
    fun `invoke returns flow that can be collected multiple times`() = runTest {
        // Arrange
        val clinics = listOf(
